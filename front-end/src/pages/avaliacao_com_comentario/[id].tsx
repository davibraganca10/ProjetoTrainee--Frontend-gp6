...
const AvaliacaoComComentario = () => {
  ...
  const [showComentarios, setShowComentarios] = useState<Record<number, boolean>>({});
        ...
        <div className="w-full max-w-md h-full min-h-screen bg-white rounded-xl shadow-md overflow-hidden p-8">
          {loadingAvaliacoes ? (
            ...
            avaliacoes.map((avaliacao) => (
              <div key={avaliacao.id} className="bg-corModal p-4 border rounded-xl mb-4">
                <div className="flex flex-rows items-center gap-2 mb-2">
                  <Image src="/perfil.png" alt="" width={30} height={40} />
                  <h2 className="text-lg font-bold text-black">{avaliacao.user.nome}</h2>
                </div>
                ...
                <div className="mt-3">
                  ...
                  >
                    {showComentarios[avaliacao.id] ? "Esconder comentários" : "Ver comentários"}
                  </button>
                </div>
